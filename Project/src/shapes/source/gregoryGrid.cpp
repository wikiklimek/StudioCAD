        ...
        const HoleCycle& hole, std::vector<std::shared_ptr<SceneObject>>& sceneObjects)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        auto p22u = std::make_shared<ScenePoint>("p22u_"+std::to_string(i), P22u_pos); p22u->Init(); sceneObjects.push_back(p22u);
        auto p22v = std::make_shared<ScenePoint>("p22v_"+std::to_string(i), P22v_pos); p22v->Init(); sceneObjects.push_back(p22v);
