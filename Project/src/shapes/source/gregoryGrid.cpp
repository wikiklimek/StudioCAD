        ...
        const HoleCycle& hole, std::vector<std::shared_ptr<SceneObject>>& sceneObjects)
{
    ...
    // NOWE: Po prostu przepisujemy gotowe ID z dziury - bo jest unikalne
    ...
    Vect3 E[3][4], IE[3][4], IIE[3][4];
    Vect3 L[3][4], R[3][4], IL[3][4], IR[3][4], IIL[3][4], IIR[3][4];
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        GetSecondInnerRow(hole.edges[i], IIE[i]);

        SubdivideBezier(E[i], L[i], R[i]);
        SubdivideBezier(IE[i], IL[i], IR[i]);
        SubdivideBezier(IIE[i], IIL[i], IIR[i]);
    }

    // krawedzie szwów miedzy 3 płatami - 3 krawedzie po 4 punkty
    Vect3 Rad[3][4];
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        Rad[i][1] = E_pt + (E_pt - I_pt); // Punkt Q_i (warunek C1 liczony z beziera t=0.5 i z wewnetzrenego rzedu beziera t=0.5)

        // Twardy warunek C2 z trzeciego rzędu płata bazowego - tez dla t=0.5
        Rad[i][2] = 4.0f * E_pt - 4.0f * I_pt + II_pt;
        //
    }

    // Punkt Centralny <=> środek ciężkości punktów Rad[2]
    Vect3 Pc_pos = (Rad[0][2] + Rad[1][2] + Rad[2][2]) / 3.0f;
    ...
    for (int i = 0; i < 3; ++i)
    {
        Rad[i][3] = Pc_pos;  // Zakończenie krzywej w centrum
    ...
    //krawedzie z bezierem - 3 lewe i 3 prawe, kazde po 4 punkty
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        // Składanie 3 sub-płatów do jednego obiektu - obramowania 1/3 czesci płatu sklejanego
        ...
        // Wymuszenie ścisłego C1 (Zewnętrzne krawędzie)
        //     - dla rozszczepionych punktów wewnetzrnych
        //       wychodzacych z łączenia z bezierem
        //       oraz ten w rogu (wychodzcay z Rad[1]) - identyczna pozycja co jego rozszczepiony kolega
        ...
        // liczymy po 2 punkty na płat (wychodzace z Rad[2])

        // Wektory kierunkowe od centrum do punktów na szwach (leżące w płaszczyźnie stycznej)
        Vect3 E_curr = Rad[curr][2] - Pc_pos;
        Vect3 E_prev = Rad[prev][2] - Pc_pos;

        // Ścisłe wyliczenie rozszczepionych punktów P22 z równań ciągłości dla n=3
